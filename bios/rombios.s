...
.byte	0
.blkb	1
! 651       { 0x4400, 0x5d00, 0x6700, 0x7100, 0
.word	$4400
.word	$5D00
.word	$6700
.word	$7100
...
mov	sp,bp
!BCC_EOS
! 1316   bios_printf(2, "apmbios " "pcibios " "pnpbios " "eltorito " "rombios32 " "\n\n");
...
mov	$148[bx],ax
!BCC_EOS
! 1535     *((Bit8u *)(&((ebda_data_t *) 0)->ata.devices[d
! 1535 evice].translation)) = (0);
! Debug: ptradd unsigned char device = [S+6-4] to [8] struct  = const $142 (used reg = )
mov	al,-2[bp]
xor	ah,ah
mov	cx,*$1E
...
!BCC_EOS
! 1680     
! 1680   *((Bit16u *)(&((ebda_data_t *) 0)->ata.devices[device].pchs.cylinders)) = (cylinders);
! Debug: ptradd unsigned char device = [S+$228-5] to [8] struct  = const $142 (used reg = )
mov	al,-3[bp]
xor	ah,ah
mov	cx,*$1E
...
mov	$14C[bx],ax
!BCC_EOS
! 1734       *((Bit16u *)(&((ebda_data_t *) 0)->ata.devices[device].lchs.cylinders)) = (cy
! 1734 linders);
! Debug: ptradd unsigned char device = [S+$228-5] to [8] struct  = const $142 (used reg = )
mov	al,-3[bp]
xor	ah,ah
...
mov	-6[bp],al
!BCC_EOS
! 1841   iobase1 = *((Bit16u *)(&((ebda_data_t *) 0)->ata.channels[channel].i
! 1841 obase1));
...
!BCC_EOS
! 2050   Bit8u status, mode, lm
! 2050 ode;
!BCC_EOS
! 2051   Bit32u total, transfer;
!BCC_EOS
...
_cdrom_boot:
! 2358   
! 2358 Bit16u ebda_seg=get_ebda_seg(), old_ds;
push	bp
mov	bp,sp
dec	sp
dec	sp
...
mov	[$25D],al
!BCC_EOS
! 2411   *((Bit8u *)(&((ebda_data_t *) 0)->cdemu.device_
! 2411 spec)) = (device%2);
...
mov	[$26C],ax
!BCC_EOS
! 2453       *((Bit16u *)(&((ebda_data_t *) 0)->cdemu.vdevice.heads)) = (_
! 2453 read_byte(446+5, boot_segment) + 1);
! Debug: list unsigned short boot_segment = [S+$820-$818] (used reg = )
push	-$816[bp]
...
!BCC_EOS
! 2822   Bit8
! 2822 u ret, mouse_data1, mouse_data2, mouse_data3;
!BCC_EOS
! 2823 ;
add	sp,*-$C
!BCC_EOS
...
!BCC_EOS
! 2888             _write_b
! 2888 yte(mouse_flags_1, &((ebda_data_t *) 0)->mouse_flag1, ebda_seg);
! Debug: list unsigned short ebda_seg = [S+$10-4] (used reg = )
push	-2[bp]
...
.419:
! 3190                                        0x00100000L,
! 3191                                        extended_memory_size - 0x00010000L, 0, 0, 1);
! Debug: list int = const 1 (used reg = )
mov	ax,*1
push	ax
! Debug: list int = const 0 (used reg = )
...
! 3297       if (scan_code !=0
.446:
! 3297  && ascii_code == 0xF0) ascii_code = 0;
! Debug: ne int = const 0 to unsigned char scan_code = [S+$C-3] (used reg = )
mov	al,-1[bp]
test	al,al
je  	.448
...
! 3369 lt:
.47A:
! 3370       bios_printf(4, "KBD: unsupported int 16h function %02x\n", *(((Bit8u *)&AX)+1));
.47C:
...
.49D:
! 3443     b
! 3443 ios_printf((2 | 4 | 1), panic_msg_keyb_buffer_full,"sendmouse");
...
jbe 	.4DB
.4DC:
! 3601         bios_printf(4, "KBD: int09
! 3601 h_handler(): unknown scancode read: 0x%02x!\n", scancode);
...
mov	-$14[bp],ax
!BCC_EOS
! 3727   Bit8u type=*((Bit8u *)(&((ebda_data_t *) 0)->ata.devi
dec	sp
! 3727 ces[device].type));
...
mov	-$15[bp],al
!BCC_EOS
! 3728   size=_read_word(SI+(Bit16u)&((dpt_t *) 0)->size, DS);
dec	sp
...
mov	-$17[bp],al
!BCC_EOS
! 3778     iobase1 = *((Bit16u *)(&((eb
! 3778 da_data_t *) 0)->ata.channels[channel].iobase1));
...
mov	$1E[bx],ax
!BCC_EOS
! 3820     *((Bit8u *)(SI+(Bit16u)&((dpt_t *) 0)->dpi.t13.dpi_length)) = (t
! 3820 13 ? 44 : 36);
mov	ax,-$12[bp]
test	ax,ax
je  	.536
...
inc	sp
!BCC_EOS
! 3821     *((Bit8u *)(SI+(Bit16u)&((dpt_t *) 0)->dpi.t13.reserved1)) = (0);
...
!BCC_EOS
! 3853       *((Bit32u *)
! 3853 (SI+(Bit16u)&((dpt_t *) 0)->dpi.t13.iface_path[4])) = (0L);
...
! 3905     case 0x01:
! 3906     
! 3906   status = _read_byte(0x0074, 0x0040);
.55F:
! Debug: list int = const $40 (used reg = )
mov	ax,*$40
push	ax
...
je  	.578
.579:
! 3947         bios_printf(4, "int13_harddisk: functi
! 3947 on %02x, error %02x !\n",*(((Bit8u *)&AX)+1),status);
...
!BCC_EOS
! 4004  
! 4004      lba_high=_read_dword(SI+(Bit16u)&((int13ext_t *) 0)->lba2, DS);
! Debug: list unsigned short DS = [S+$26+4] (used reg = )
push	6[bp]
...
! 4059     case 0x14:
.5AA:
! 4060       bios_printf(4, "int13_harddisk: functio
.5AB:
! 4060 n %02xh unimplemented, returns success\n", *(((Bit8u *)&AX)+1));
! Debug: list unsigned char AX = [S+$26+$17] (used reg = )
mov	al,$19[bp]
xor	ah,ah
push	ax
...
! 4268 int13_eltorito(DS, ES, DI, SI, BP, SP, BX, DX, CX, AX, IP, CS, FLAG
! 4268 S)
! 4269   Bit16u DS, ES, DI, SI, BP, SP, BX, DX, CX, AX, IP, CS, FLAGS;
...
! 4315   Bit16u DS, ES, DI, SI, BP, S
export	_int13_cdemu
_int13_cdemu:
! 4315 P, BX, DX, CX, AX, IP, CS, FLAGS;
!BCC_EOS
! 4316 {
...
mov	-$2C[bp],ax
!BCC_EOS
! 4376       vlba=((((Bit32u)cylinder*(Bit32u)vheads)+(Bit32u)head)*(Bit32u)vspt)+((Bit32u)(sector-1));
...
mov	-$15[bp],al
!BCC_EOS
! 4789       return_status[3] = inb(0x03f5);
! Debug: list int = const $3F5 (used reg = )
mov	ax,#$3F5
push	ax
! Debug: func () unsigned char = inb+0 (used reg = )
...
! 5098       }
! 5099       i
! 5099 f (floppy_drive_exists(drive) == 0) {
.72A:
! Debug: list unsigned char drive = [S+$24-3] (used reg = )
mov	al,-1[bp]
xor	ah,ah
...
mov	$11[bp],al
!BCC_EOS
! 5444       regs.u.r8.dl = inb_cmos(0x0b) & 0x01;
! Debug: list int = const $B (used reg = )
mov	ax,*$B
push	ax
! Debug: func () unsigned char = inb_cmos+0 (used reg = )
...
! 8415 endasm
!BCC_ENDASM
! 8416 static Bit8u vgafont8[128*8]=
! Register BX used in function int70_function
! 8417 {
.data
_vgafont8:
! 8418  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
.byte	0
.byte	0
.byte	0
.byte	0
...
.byte	0
.byte	0
! 8419  0x7e, 0x81, 0xa5, 0x81, 0xbd, 0x99, 0x81, 0x7e,
.byte	$7E
.byte	$81
.byte	$A5
.byte	$81
...
.byte	$81
.byte	$7E
! 8420  0x7e, 0xff, 0xdb, 0xff, 0xc3, 0xe7, 0xff, 0x7e,
.byte	$7E
.byte	$FF
.byte	$DB
.byte	$FF
...
.byte	$FF
.byte	$7E
! 8421  0x6c, 0xfe, 0xfe, 0xfe, 0x7c, 0x38, 0x10, 0x00,
.byte	$6C
.byte	$FE
.byte	$FE
.byte	$FE
...
.byte	$10
.byte	0
! 8422  0x10, 0x38, 0x7c, 0xfe, 0x7c, 0x38, 0x10, 0x00,
.byte	$10
.byte	$38
.byte	$7C
.byte	$FE
...
.byte	$10
.byte	0
! 8423  0x38, 0x7c, 0x38, 0xfe, 
.byte	$38
.byte	$7C
.byte	$38
.byte	$FE
! 8423 0xfe, 0x7c, 0x38, 0x7c,
.byte	$FE
.byte	$7C
.byte	$38
.byte	$7C
! 8424  0x10, 0x10, 0x38, 0x7c, 0xfe, 0x7c, 0x38, 0x7c,
.byte	$10
.byte	$10
.byte	$38
.byte	$7C
...
.byte	$38
.byte	$7C
! 8425  0x00, 0x00, 0x18, 0x3c, 0x3c, 0x18, 0x00, 0x00,
.byte	0
.byte	0
.byte	$18
.byte	$3C
...
.byte	0
.byte	0
! 8426  0xff, 0xff, 0xe7, 0xc3, 0xc3, 0xe7, 0xff, 0xff,
.byte	$FF
.byte	$FF
.byte	$E7
.byte	$C3
...
.byte	$FF
.byte	$FF
! 8427  0x00, 0x3c, 0x66, 0x42, 0x42, 0x66, 0x3c, 0x00,
.byte	0
.byte	$3C
.byte	$66
.byte	$42
...
.byte	$3C
.byte	0
! 8428  0xff, 0xc3, 0x99, 0xbd, 0xbd, 0x99, 0xc3, 0xff,
.byte	$FF
.byte	$C3
.byte	$99
.byte	$BD
...
.byte	$C3
.byte	$FF
! 8429  0x0f, 0x07, 0x0f, 0x7d, 0xcc, 0xcc, 0xcc, 0x78,
.byte	$F
.byte	7
.byte	$F
.byte	$7D
...
.byte	$CC
.byte	$78
! 8430  0x3c, 0x66, 0x66, 0x66, 0x3c, 0x18, 0x7e, 0x18,
.byte	$3C
.byte	$66
.byte	$66
.byte	$66
...
.byte	$7E
.byte	$18
! 8431  0x3f, 0x33, 0x3f, 0x30, 0x30, 0x70, 0xf0, 0xe0,
.byte	$3F
.byte	$33
.byte	$3F
.byte	$30
...
.byte	$F0
.byte	$E0
! 8432  0x7f, 0x63, 0x7f, 0x63, 0x63, 0x67, 0xe6, 0xc0,
.byte	$7F
.byte	$63
.byte	$7F
.byte	$63
...
.byte	$E6
.byte	$C0
! 8433  0x99, 0x5a, 0x3c, 0xe7, 0xe7, 0x3c, 0x5a, 0x99,
.byte	$99
.byte	$5A
.byte	$3C
.byte	$E7
...
.byte	$5A
.byte	$99
! 8434  0x80, 0xe0, 0xf8, 0xfe, 0xf8, 0xe0, 0x80, 0x00,
.byte	$80
.byte	$E0
.byte	$F8
.byte	$FE
...
.byte	$80
.byte	0
! 8435  0x02, 0x0e, 0x3e, 0xfe, 0x3e, 0x0e, 0x02, 0x00,
.byte	2
.byte	$E
.byte	$3E
.byte	$FE
...
.byte	2
.byte	0
! 8436  0x18, 0x3c, 0x7e, 0x18, 0x18, 0x7e, 0x3c, 0x18,
.byte	$18
.byte	$3C
.byte	$7E
.byte	$18
...
.byte	$3C
.byte	$18
! 8437  0x66, 0x66, 0x66, 0x66, 0x66, 0x00, 0x66, 0x00,
.byte	$66
.byte	$66
.byte	$66
.byte	$66
...
.byte	$66
.byte	0
! 8438  0x7f, 0xdb, 0xdb, 0x7b, 0x1b, 0x1b, 0x1b, 0x00,
.byte	$7F
.byte	$DB
.byte	$DB
.byte	$7B
...
.byte	$1B
.byte	0
! 8439  0x3e, 0x63, 0x38, 0x6c, 0x6c, 0x38, 0xcc, 0x78,
.byte	$3E
.byte	$63
.byte	$38
.byte	$6C
...
.byte	$CC
.byte	$78
! 8440  0x00, 0x00, 0x00, 0x00, 0x7e, 0x7e, 0x7e, 0x00,
.byte	0
.byte	0
.byte	0
.byte	0
...
.byte	$7E
.byte	0
! 8441  0x18, 0x3c, 0x7e, 0x18, 0x7e, 0x3c, 0x18, 0xff,
.byte	$18
.byte	$3C
.byte	$7E
.byte	$18
...
.byte	$18
.byte	$FF
! 8442  0x18, 0x3c, 0x7e, 0x18, 0x18, 0x18, 0x18, 0x00,
.byte	$18
.byte	$3C
.byte	$7E
.byte	$18
...
.byte	$18
.byte	0
! 8443  0x18, 0x18, 0x18, 0x18, 0x7e, 0x3c, 0x18, 0x00,
.byte	$18
.byte	$18
.byte	$18
.byte	$18
...
.byte	$18
.byte	0
! 8444  0x00, 0x18, 0x0c, 0xfe, 0x0c, 0x18, 0x00, 0x00,
.byte	0
.byte	$18
.byte	$C
.byte	$FE
...
.byte	0
.byte	0
! 8445  0x00, 0x30, 0x60, 0xfe, 0x60, 0x30, 0x00, 0x00,
.byte	0
.byte	$30
.byte	$60
.byte	$FE
...
.byte	0
.byte	0
! 8446  0x00, 0x00, 0xc0, 0xc0, 0xc0, 0xfe, 0x00, 0x00,
.byte	0
.byte	0
.byte	$C0
.byte	$C0
...
.byte	0
.byte	0
! 8447  0x00, 0x24, 0x66, 0xff, 0x66, 0x24, 0x00, 0x00,
.byte	0
.byte	$24
.byte	$66
.byte	$FF
...
.byte	0
.byte	0
! 8448  0x00, 0x18, 0x3c, 0x7e, 0xff, 0xff, 0x00, 0x00,
.byte	0
.byte	$18
.byte	$3C
.byte	$7E
...
.byte	0
.byte	0
! 8449  0x00, 0xff, 0xff, 0x7e, 0x3c, 0x18, 0x00, 0x00,
.byte	0
.byte	$FF
.byte	$FF
.byte	$7E
...
.byte	0
.byte	0
! 8451  0x30, 0x78, 0x78, 0x30, 0x30, 0x00, 0x30, 0x00,
.byte	$30
.byte	$78
.byte	$78
.byte	$30
...
.byte	$30
.byte	0
! 8452  0x6c, 0x6c, 0x6c, 0x00, 0x00, 0x00, 0x00, 0x00,
.byte	$6C
.byte	$6C
.byte	$6C
.byte	0
...
.byte	0
.byte	0
! 8453  0x6c, 0x6c, 0xfe, 0x6c, 0xfe, 0x6c, 0x6c, 0x00,
.byte	$6C
.byte	$6C
.byte	$FE
.byte	$6C
...
.byte	$6C
.byte	0
! 8454  0x30, 0x7c, 0xc0, 0x78, 0x0c, 0xf8, 0x30, 0x00,
.byte	$30
.byte	$7C
.byte	$C0
.byte	$78
...
.byte	$30
.byte	0
! 8455  0x00, 0xc6, 0xcc, 0x18, 0x30, 0x66, 0xc6, 0x00,
.byte	0
.byte	$C6
.byte	$CC
.byte	$18
...
.byte	$C6
.byte	0
! 8456  0x38, 0x6c, 0x38, 0x76, 0xdc, 0xcc, 0x76, 0x00,
.byte	$38
.byte	$6C
.byte	$38
.byte	$76
...
.byte	$76
.byte	0
! 8457  0x60, 0x60, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00,
.byte	$60
.byte	$60
.byte	$C0
.byte	0
...
.byte	0
.byte	0
! 8458  0x18, 0x30, 0x60, 0x60, 0x60, 0x30, 0x18, 0x00,
.byte	$18
.byte	$30
.byte	$60
.byte	$60
...
.byte	$18
.byte	0
! 8459  0x60, 0x30, 0x18, 0x18, 0x18, 0x30, 0x60, 0x00,
.byte	$60
.byte	$30
.byte	$18
.byte	$18
...
.byte	$60
.byte	0
! 8460  0x00, 0x66, 0x3c, 0xff, 0x3c, 0x66, 0x00, 0x00,
.byte	0
.byte	$66
.byte	$3C
.byte	$FF
...
.byte	0
.byte	0
! 8461  0x00, 0x30, 0x30, 0xfc, 0x30, 0x30, 0x00, 0x00,
.byte	0
.byte	$30
.byte	$30
.byte	$FC
...
.byte	0
.byte	0
! 8462  0x00, 0x00, 0x00, 0x00, 0x00, 0x30, 0x30, 0x60,
.byte	0
.byte	0
.byte	0
.byte	0
...
.byte	$30
.byte	$60
! 8463  0x00, 0x00, 0x00, 0xfc, 0x00, 0x00, 0x00, 0x00,
.byte	0
.byte	0
.byte	0
.byte	$FC
...
.byte	0
.byte	0
! 8464  0x00, 0x00, 0x00, 0x00, 0x00, 0x30, 0x30, 0x00,
.byte	0
.byte	0
.byte	0
.byte	0
...
.byte	6
.byte	$C
! 8465 18, 0x30, 0x60, 0xc0, 0x80, 0x00,
.byte	$18
.byte	$30
.byte	$60
.byte	$C0
.byte	$80
.byte	0
! 8466  0x7c, 0xc6, 0xce, 0xde, 0xf6, 0xe6, 0x7c, 0x00,
.byte	$7C
.byte	$C6
.byte	$CE
.byte	$DE
...
.byte	$7C
.byte	0
! 8467  0x30, 0x70, 0x30, 0x30, 0x30, 0x30, 0xfc, 0x00,
.byte	$30
.byte	$70
.byte	$30
.byte	$30
...
.byte	$FC
.byte	0
! 8468  0x78, 0xcc, 0x0c, 0x38, 0x60, 0xcc, 0xfc, 0x00,
.byte	$78
.byte	$CC
.byte	$C
.byte	$38
...
.byte	$FC
.byte	0
! 8469  0x78, 0xcc, 0x0c, 0x38, 0x0c, 0xcc, 0x78, 0x00,
.byte	$78
.byte	$CC
.byte	$C
.byte	$38
...
.byte	$78
.byte	0
! 8470  0x1c, 0x3c, 0x6c, 0xcc, 0xfe, 0x0c, 0x1e, 0x00,
.byte	$1C
.byte	$3C
.byte	$6C
.byte	$CC
...
.byte	$1E
.byte	0
! 8471  0xfc, 0xc0, 0xf8, 0x0c, 0x0c, 0xcc, 0x78, 0x00,
.byte	$FC
.byte	$C0
.byte	$F8
.byte	$C
...
.byte	$78
.byte	0
! 8472  0x38, 0x60, 0xc0, 0xf8, 0xcc, 0xcc, 0x78, 0x00,
.byte	$38
.byte	$60
.byte	$C0
.byte	$F8
...
.byte	$78
.byte	0
! 8473  0xfc, 0xcc, 0x0c, 0x18, 0x30, 0x30, 0x30, 0x00,
.byte	$FC
.byte	$CC
.byte	$C
.byte	$18
...
.byte	$30
.byte	0
! 8474  0x78, 0xcc, 0xcc, 0x78, 0xcc, 0xcc, 0x78, 0x00,
.byte	$78
.byte	$CC
.byte	$CC
.byte	$78
...
.byte	$78
.byte	0
! 8475  0x78, 0xcc, 0xcc, 0x7c, 0x0c, 0x18, 0x70, 0x00,
.byte	$78
.byte	$CC
.byte	$CC
.byte	$7C
...
.byte	$70
.byte	0
! 8476  0x00, 0x30, 0x30, 0x00, 0x00, 0x30, 0x30, 0x00,
.byte	0
.byte	$30
.byte	$30
.byte	0
...
.byte	$30
.byte	0
! 8477  0x00, 0x30, 0x30, 0x00, 0x00, 0x30, 0x30, 0x60,
.byte	0
.byte	$30
.byte	$30
.byte	0
...
.byte	$30
.byte	$60
! 8478  0x18, 0x30, 0x60, 0xc0, 0x60, 0x30, 0x18, 0x00,
.byte	$18
.byte	$30
.byte	$60
.byte	$C0
...
.byte	$18
.byte	0
! 8479  0x00, 0x00, 0xfc, 0x00, 0x00, 0xfc, 0x00, 0x00,
.byte	0
.byte	0
.byte	$FC
.byte	0
...
.byte	0
.byte	0
! 8480  0x60, 0x30, 0x18, 0x0c, 0x18, 0x30, 0x60, 0x00,
.byte	$60
.byte	$30
.byte	$18
.byte	$C
...
.byte	$60
.byte	0
! 8481  0x78, 0xcc, 0x0c, 0x18, 0x30, 0x00, 0x30, 0x00,
.byte	$78
.byte	$CC
.byte	$C
.byte	$18
...
.byte	$30
.byte	0
! 8482  0x7c, 0xc6, 0xde, 0xde, 0xde, 0xc0, 0x78, 0x00,
.byte	$7C
.byte	$C6
.byte	$DE
.byte	$DE
...
.byte	$78
.byte	0
! 8483  0x30, 0x78, 0xcc, 0xcc, 0xfc, 0xcc, 0xcc, 0x00,
.byte	$30
.byte	$78
.byte	$CC
.byte	$CC
...
.byte	$CC
.byte	0
! 8484  0xfc, 0x66, 0x66, 0x7c, 0x66, 0x66, 0xfc, 0x00,
.byte	$FC
.byte	$66
.byte	$66
.byte	$7C
...
.byte	$FC
.byte	0
! 8485  0x3c, 0x66, 0xc0, 0xc0, 0xc0, 0x66, 0x3c, 0x00,
.byte	$3C
.byte	$66
.byte	$C0
.byte	$C0
...
.byte	$3C
.byte	0
! 8486  0xf8, 0x6c, 0x66, 0x66, 0x66, 0x6c, 0xf8, 0x00,
.byte	$F8
.byte	$6C
.byte	$66
.byte	$66
...
.byte	$F8
.byte	0
! 8487  0xfe, 0x62, 0x68, 0x78, 0x68, 0x62, 0xfe, 0x00,
.byte	$FE
.byte	$62
.byte	$68
.byte	$78
...
.byte	$FE
.byte	0
! 8488  0xfe, 0x62, 0x68, 0x78, 0x68, 0x60, 0xf0, 0x00,
.byte	$FE
.byte	$62
.byte	$68
.byte	$78
...
.byte	$F0
.byte	0
! 8489  0x3c, 0x66, 0xc0, 0xc0, 0xce, 0x66, 0x3e, 0x00,
.byte	$3C
.byte	$66
.byte	$C0
.byte	$C0
...
.byte	$3E
.byte	0
! 8490  0xcc, 0xcc, 0xcc, 0xfc, 0xcc, 0xcc, 0xcc, 0x00,
.byte	$CC
.byte	$CC
.byte	$CC
.byte	$FC
...
.byte	$CC
.byte	0
! 8491  0x78, 0x30, 0x30, 0x30, 0x30, 0x30, 0x78, 0x00,
.byte	$78
.byte	$30
.byte	$30
.byte	$30
...
.byte	$78
.byte	0
! 8492  0x1e, 0x0c, 0x0c, 0x0c, 0xcc, 0xcc, 0x78, 0x00,
.byte	$1E
.byte	$C
.byte	$C
.byte	$C
...
.byte	$78
.byte	0
! 8493  0xe6, 0x66, 0x6c, 0x78, 0x6c, 0x66, 0xe6, 0x00,
.byte	$E6
.byte	$66
.byte	$6C
.byte	$78
...
.byte	$E6
.byte	0
! 8494  0xf0, 0x60, 0x60, 0x60, 0x62, 0x66, 0xfe, 0x00,
.byte	$F0
.byte	$60
.byte	$60
.byte	$60
...
.byte	$FE
.byte	0
! 8495  0xc6, 0xee, 0xfe, 0xfe, 0xd6, 0xc6, 0xc6, 0x00,
.byte	$C6
.byte	$EE
.byte	$FE
.byte	$FE
...
.byte	$C6
.byte	0
! 8496  0xc6, 0xe6, 0xf6, 0xde, 0xce, 0xc6, 0xc6, 0x00,
.byte	$C6
.byte	$E6
.byte	$F6
.byte	$DE
...
.byte	$C6
.byte	0
! 8497  0x38, 0x6c, 0xc6, 0xc6, 0xc6, 0x6c, 0x38, 0x00,
.byte	$38
.byte	$6C
.byte	$C6
.byte	$C6
...
.byte	$38
.byte	0
! 8498  0xfc, 0x66, 0x66, 0x7c, 0x60, 0x60, 0xf0, 0x00,
.byte	$FC
.byte	$66
.byte	$66
.byte	$7C
...
.byte	$F0
.byte	0
! 8499  0x78, 0xcc, 0xcc, 0xcc, 0xdc, 0x78, 0x1c, 0x00,
.byte	$78
.byte	$CC
.byte	$CC
.byte	$CC
...
.byte	$1C
.byte	0
! 8500  0xfc, 0x66, 0x66, 0x7c, 0x6c, 0x66, 0xe6, 0x00,
.byte	$FC
.byte	$66
.byte	$66
.byte	$7C
...
.byte	$E6
.byte	0
! 8501  0x78, 0xcc, 0xe0, 0x70, 0x1c, 0xcc, 0x78, 0x00,
.byte	$78
.byte	$CC
.byte	$E0
.byte	$70
...
.byte	$78
.byte	0
! 8502  0xfc, 0xb4, 0x30, 0x30, 0x30, 0x30, 0x78, 0x00,
.byte	$FC
.byte	$B4
.byte	$30
.byte	$30
...
.byte	$78
.byte	0
! 8503  0xcc, 0xcc, 0xcc, 0xcc, 0xcc, 0xcc, 0xfc, 0x00,
.byte	$CC
.byte	$CC
.byte	$CC
.byte	$CC
...
.byte	$FC
.byte	0
! 8504  0xcc, 0xcc, 0xcc, 0xcc, 0xcc, 0x78, 0x30, 0x00,
.byte	$CC
.byte	$CC
.byte	$CC
.byte	$CC
...
.byte	$30
.byte	0
! 8505  0xc6, 0xc6, 0xc6, 0xd6, 0xfe, 0xee, 0xc6, 0x00,
.byte	$C6
.byte	$C6
.byte	$C6
.byte	$D6
...
.byte	$C6
.byte	0
! 8506  0xc6, 0xc6, 0x6c, 0x38, 0x38, 0x6c, 0xc6, 0x00,
.byte	$C6
.byte	$C6
.byte	$6C
.byte	$38
...
.byte	0
! 8507  0xcc
! 8507 , 0xcc, 0xcc, 0x78, 0x30, 0x30, 0x78, 0x00,
.byte	$CC
.byte	$CC
.byte	$CC
.byte	$78
...
.byte	$78
.byte	0
! 8508  0xfe, 0xc6, 0x8c, 0x18, 0x32, 0x66, 0xfe, 0x00,
.byte	$FE
.byte	$C6
.byte	$8C
.byte	$18
...
.byte	$FE
.byte	0
! 8509  0x78, 0x60, 0x60, 0x60, 0x60, 0x60, 0x78, 0x00,
.byte	$78
.byte	$60
.byte	$60
.byte	$60
...
.byte	$78
.byte	0
! 8510  0xc0, 0x60, 0x30, 0x18, 0x0c, 0x06, 0x02, 0x00,
.byte	$C0
.byte	$60
.byte	$30
.byte	$18
...
.byte	2
.byte	0
! 8511  0x78, 0x18, 0x18, 0x18, 0x18, 0x18, 0x78, 0x00,
.byte	$78
.byte	$18
.byte	$18
.byte	$18
...
.byte	$78
.byte	0
! 8512  0x10, 0x38, 0x6c, 0xc6, 0x00, 0x00, 0x00, 0x00,
.byte	$10
.byte	$38
.byte	$6C
.byte	$C6
...
.byte	0
.byte	0
! 8513  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff,
.byte	0
.byte	0
.byte	0
.byte	0
...
.byte	0
.byte	$FF
! 8514  0x30, 0x30, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00,
.byte	$30
.byte	$30
.byte	$18
.byte	0
...
.byte	0
.byte	0
! 8515  0x00, 0x00, 0x78, 0x0c, 0x7c, 0xcc, 0x76, 0x00,
.byte	0
.byte	0
.byte	$78
.byte	$C
...
.byte	$76
.byte	0
! 8516  0xe0, 0x60, 0x60, 0x7c, 0x66, 0x66, 0xdc, 0x00,
.byte	$E0
.byte	$60
.byte	$60
.byte	$7C
...
.byte	$DC
.byte	0
! 8517  0x00, 0x00, 0x78, 0xcc, 0xc0, 0xcc, 0x78, 0x00,
.byte	0
.byte	0
.byte	$78
.byte	$CC
...
.byte	$78
.byte	0
! 8518  0x1c, 0x0c, 0x0c, 0x7c, 0xcc, 0xcc, 0x76, 0x00,
.byte	$1C
.byte	$C
.byte	$C
.byte	$7C
...
.byte	$76
.byte	0
! 8519  0x00, 0x00, 0x78, 0xcc, 0xfc, 0xc0, 0x78, 0x00,
.byte	0
.byte	0
.byte	$78
.byte	$CC
...
.byte	$78
.byte	0
! 8520  0x38, 0x6c, 0x60, 0xf0, 0x60, 0x60, 0xf0, 0x00,
.byte	$38
.byte	$6C
.byte	$60
.byte	$F0
...
.byte	$F0
.byte	0
! 8521  0x00, 0x00, 0x76, 0xcc, 0xcc, 0x7c, 0x0c, 0xf8,
.byte	0
.byte	0
.byte	$76
.byte	$CC
...
.byte	$C
.byte	$F8
! 8522  0xe0, 0x60, 0x6c, 0x76, 0x66, 0x66, 0xe6, 0x00,
.byte	$E0
.byte	$60
.byte	$6C
.byte	$76
...
.byte	$E6
.byte	0
! 8523  0x30, 0x00, 0x70, 0x30, 0x30, 0x30, 0x78, 0x00,
.byte	$30
.byte	0
.byte	$70
.byte	$30
...
.byte	$78
.byte	0
! 8524  0x0c, 0x00, 0x0c, 0x0c, 0x0c, 0xcc, 0xcc, 0x78,
.byte	$C
.byte	0
.byte	$C
.byte	$C
...
.byte	$CC
.byte	$78
! 8525  0xe0, 0x60, 0x66, 0x6c, 0x78, 0x6c, 0xe6, 0x00,
.byte	$E0
.byte	$60
.byte	$66
.byte	$6C
...
.byte	$E6
.byte	0
! 8526  0x70, 0x30, 0x30, 0x30, 0x30, 0x30, 0x78, 0x00,
.byte	$70
.byte	$30
.byte	$30
.byte	$30
...
.byte	$78
.byte	0
! 8527  0x00, 0x00, 0xcc, 0xfe, 0xfe, 0xd6, 0xc6, 0x00,
.byte	0
.byte	0
.byte	$CC
.byte	$FE
...
.byte	$C6
.byte	0
! 8528  0x00, 0x00, 0xf8, 0xcc, 0xcc, 0xcc, 0xcc, 0x00,
.byte	0
.byte	0
.byte	$F8
.byte	$CC
...
.byte	$CC
.byte	0
! 8529  0x00, 0x00, 0x78, 0xcc, 0xcc, 0xcc, 0x78, 0x00,
.byte	0
.byte	0
.byte	$78
.byte	$CC
...
.byte	$78
.byte	0
! 8530  0x00, 0x00, 0xdc, 0x66, 0x66, 0x7c, 0x60, 0xf0,
.byte	0
.byte	0
.byte	$DC
.byte	$66
...
.byte	$60
.byte	$F0
! 8531  0x00, 0x00, 0x76, 0xcc, 0xcc, 0x7c, 0x0c, 0x1e,
.byte	0
.byte	0
.byte	$76
.byte	$CC
...
.byte	$C
.byte	$1E
! 8532  0x00, 0x00, 0xdc, 0x76, 0x66, 0x60, 0xf0, 0x00,
.byte	0
.byte	0
.byte	$DC
.byte	$76
...
.byte	$F0
.byte	0
! 8533  0x00, 0x00, 0x7c, 0xc0, 0x78, 0x0c, 0xf8, 0x00,
.byte	0
.byte	0
.byte	$7C
.byte	$C0
...
.byte	$F8
.byte	0
! 8534  0x10, 0x30, 0x7c, 0x30, 0x30, 0x34, 0x18, 0x00,
.byte	$10
.byte	$30
.byte	$7C
.byte	$30
...
.byte	$18
.byte	0
! 8535  0x00, 0x00, 0xcc, 0xcc, 0xcc, 0xcc, 0x76, 0x00,
.byte	0
.byte	0
.byte	$CC
.byte	$CC
...
.byte	$76
.byte	0
! 8536  0x00, 0x00, 0xcc, 0xcc, 0xcc, 0x78, 0x30, 0x00,
.byte	0
.byte	0
.byte	$CC
.byte	$CC
...
.byte	$30
.byte	0
! 8537  0x00, 0x00, 0xc6, 0xd6, 0xfe, 0xfe, 0x6c, 0x00,
.byte	0
.byte	0
.byte	$C6
.byte	$D6
...
.byte	$6C
.byte	0
! 8538  0x00, 0x00, 0xc6, 0x6c, 0x38, 0x6c, 0xc6, 0x00,
.byte	0
.byte	0
.byte	$C6
.byte	$6C
...
.byte	$C6
.byte	0
! 8539  0x00, 0x00, 0xcc, 0xcc, 0xcc, 0x7c, 0x0c, 0xf8,
.byte	0
.byte	0
.byte	$CC
.byte	$CC
...
.byte	$C
.byte	$F8
! 8540  0x00, 0x00, 0xfc, 0x98, 0x30, 0x64, 0xfc, 0x00,
.byte	0
.byte	0
.byte	$FC
.byte	$98
...
.byte	$FC
.byte	0
! 8541  0x1c, 0x30, 0x30, 0xe0, 0x30, 0x30, 0x1c, 0x00,
.byte	$1C
.byte	$30
.byte	$30
.byte	$E0
...
.byte	$1C
.byte	0
! 8542  0x18, 0x18, 0x18, 0x00, 0x18, 0x18, 0x18, 0x00,
.byte	$18
.byte	$18
.byte	$18
.byte	0
...
.byte	$18
.byte	0
! 8543  0xe0, 0x30, 0x30, 0x1c, 0x30, 0x30, 0xe0, 0x00,
.byte	$E0
.byte	$30
.byte	$30
.byte	$1C
...
.byte	$E0
.byte	0
! 8544  0x76, 0xdc, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
.byte	$76
.byte	$DC
.byte	0
.byte	0
...
.byte	0
.byte	0
! 8545  0x00, 0x10, 0x38, 0x6c, 0xc6, 0xc6, 0xfe, 0x00,
.byte	0
.byte	$10
.byte	$38
.byte	$6C
