...
int BtnLkup[7] = {
    CMD_CONT, CMD_NEXT,CMD_STEP1, CMD_STEPN, CMD_RFRSH, CMD_BREAK
};
...
Bit64u BottomAsmLA =0;         // beginning and end addrs on ASM window
Bit64u TopAsmLA =0;

Bit32u PrevStepNSize = 50;  // cpu_loop control variables
unsigned TotCPUs =0;           // # of CPUs in a multi-CPU simulation
unsigned CpuSupportSSE = 0; // cpu supports SSE
unsigned CurrentCPU =0;        // cpu that is being displayed

struct ASKTEXT ask_str={0};

// window resizing/docking stuff
int OneCharWide =0;    // average width of a char in current font (pixels)
...
bool doSimuInit =0;             // Internal flag #2
bool ResizeColmns =0;           // address/value column autosize flag
bool FWflag = FALSE;         // friendly warning has been shown to user once already

static char *PrevStack =0;             // buffer for testing changes in stack values
Bit64u PStackLA = 0;                // to calculate alignment between prev and current stack
bool StackEntChg[STACK_ENTRIES]={0};     // flag for "change detected" on each stack line
...
static char* RDispName[EFER_Rnum + 1]={0};
static bx_param_num_c *RegObject[BX_MAX_SMP_THREADS_SUPPORTED][TOT_REG_NUM + EXTRA_REGS];
Bit64u rV[EFER_Rnum + 1]={0};   // current values of registers
Bit64u PV[EFER_Rnum + 1]={0};   // previous values of registers
Bit32s GDT_Len=0;             // "limits" (= bytesize-1) for GDT and IDT
Bit32s IDT_Len=0;
Bit8u RegColor[TOT_REG_NUM]={0};    // specifies foreground and background color of registers
// Text color is red if the upper bit is set. Background is set according to ColorList.
int RitemToRnum[TOT_REG_NUM]={0};   // mapping from Reg List Item# to register number
...
int DumpAlign = 1;
int PrevDAD=0;                   // saves "previous DumpAlign value" (forces column autosize)
char *DataDump=0;
Bit64u DumpStart = 0;          // current emulated address (lin or phys) of DataDump
bool doDumpRefresh=0;
int DViewMode = VIEW_MEMDUMP;
bool LinearDump = TRUE;        // FALSE = memdump uses physical addressing

char *tmpcb=0;                   // 512b is allocated in bigbuf
char *CurStack=0;                // Stack workspace (400b usually)
char AsciiHex[512]={0};            // Unsigned char to printable hex xlat table
static char UCtable[256]={0};

char bigbuf[outbufSIZE]={0};       // 40K preallocated storage for all char buffers (see DoAllInit)
char *DbgAppendPtr = bigbuf;
char *OutWindow=0;               // buffer for the Output window
...
int AsmPgSize = 0;
int ListLineRatio=0;             // number of vertical pixels in a ListView Item
int ListVerticalPix=0;           // number of vertical pixels in each List
Bit64u AsmLA[MAX_ASM]={0};         // linear address of each disassembled ASM line
int AsmLine[MAX_ASM]={0};

// Command stuff
int CommandHistoryIdx = 0;
char *CmdHistory[CmdHistorySize]={0};  // 64 command History storage (fixed 80b each)
...
bx_address BrkLAddr[BX_DBG_MAX_LIN_BPOINTS+1]={0};
unsigned BrkIdx[BX_DBG_MAX_LIN_BPOINTS+1]={0};
int BreakCount = 0;

// Breakpoint Dump Window stuff
unsigned short BrkpIDMap[256]={0};
unsigned short WWP_BaseEntry=0;
unsigned short RWP_BaseEntry=0;
unsigned short EndLinEntry=0;
unsigned short EndPhyEntry=0;
unsigned short WWPSnapCount=0;
unsigned short RWPSnapCount=0;
bx_phy_address WWP_Snapshot[16]={0};
bx_phy_address RWP_Snapshot[16]={0};

char *debug_cmd=0;
bool debug_cmd_ready=false;
bool vgaw_refresh=false;
...
void ParseIDText(const char *text)
{
    int i = 0;
    int overflow = 0;
    while (*text !=0 && *text != '\r' && *text != '\n' && DbgAppendPtr < bigbuf+(sizeof bigbuf))
        *(DbgAppendPtr++)= *(text++);  // append the chars from x into the bigbuf
    if (DbgAppendPtr >= bigbuf+(sizeof bigbuf))      // overflow error?
    {
        PO_Tdelay = 2;
    ...
    *DbgAppendPtr = 0;
    if (*text == 0)            // automatically process only complete lines further
    {
...
void FillSrc()
{
    ...
    StartListUpdate(SRCT_WND);
    for(unsigned i =0;i<lines.size();i++)
    {
        snprintf(col0,sizeof(col0),"%8d",i+1);
        cols[1]=(char*)lines[i].c_str();
        InsertListRow(cols, 3, SRCT_WND, i, 8); 
    }
...
#ifdef USE_SRCSHOW
                AsmLine[AsmLineCount]=0;
            if(translate_address(asmtxt)){                        
                unsigned int ln = get_line();
                const char* fi = get_filename();
                const char* fn = get_functionname();
                if(fi!=0)
                {
                    if(on_file(fi,lines))
                    {
...
void FillPAGE()
{
    ...
    start_phy = 2;
    while (LineCount < 1024*1024 /*&& curlin != 0xfffff000*/)
    {
...
void OnBreak()
{
    ...
    if (In64Mode == FALSE)      // "display" linear addy of the opcode
        ...
        sprintf (asmtxt,Fmt64b[UprCase],CurrentAsmLA);
#ifdef USE_SRCSHOW
    if(translate_address(asmtxt)){                        
        unsigned int ln = get_line();
        const char* fi = get_filename();
...
bool InitDataDump(bool isLinear, Bit64u newDS)
{
    ...
    if (isLinear)
    {
        ...
        retval = (bool) bx_mem.dbg_fetch_mem(BX_CPU(CurrentCPU),
            (bx_phy_address)newDS,1024, (Bit8u *)DataDump);
    }
    if (retval == FALSE)
    {
...
void doStepN()
{
    ...
    if (AskText("Singlestep N times","Number of debug steps (use 0x for hex):",tmpcb) == FALSE)
        return;
            ...
            if (AtBreak != FALSE && debug_cmd_ready == FALSE)
            {
                *tmpcb = 0;
                ...
                else
                {
                    ...
                    if(tmpcb!=0&&tmpcb[0]==':')
                    {
                        //:/home/yilin/Working/linux-0.11-m/linux-0.11-gcc-9.4.0/tools/system
                        load_bfd(tmpcb+1);
                    }else
                    {
