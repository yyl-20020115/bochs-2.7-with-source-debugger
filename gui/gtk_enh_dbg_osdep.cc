...
void MakeBL(TreeParent *tp, bx_param_c *p,char* name);
...
GtkWidget *Cmd2MI[CMD_IDX_HI - CMD_IDX_LO + 1]={0};

// GTK objects and global stuff
GtkTextMark* EndMark=0;
GtkTreeViewColumn *AllCols[27]={0};
GtkCellRenderer *LV_Rend[4]={0};
GdkCursor *SizeCurs=0;
GdkCursor *DockCurs=0;
gulong mmov_hID=0;
...
GtkWidget *menubar=0;
GtkWidget *CmdMenu=0;
GtkWidget *ViewMenu=0;
GtkWidget *OptMenu=0;
GtkWidget *HelpMenu=0;
GtkWidget *MainVbox=0;
GtkWidget *CmdBHbox=0;
GtkWidget *CpuBHbox=0;
GtkWidget *StatHbox=0;
GtkWidget *TreeTbl=0;
GtkWidget *IOVbox=0;
GtkWidget *ScrlWinOut=0;
GtkWidget *ScrlWin[4]={0};
GtkWidget *LVHbox=0;
GtkWidget *LVEvtBox=0;
GtkWidget *VSepEvtBox1=0;
GtkWidget *VSepEvtBox2=0;
GtkWidget *VSepEvtBox3=0;

    // GTK menu widgets -- titles, then actual menu items
GtkWidget *CmdTitle=0;
GtkWidget *ViewTitle=0;
GtkWidget *OptTitle=0;
GtkWidget *HelpTitle=0;

GtkWidget *ContMI=0;
GtkWidget *StepMI=0;
GtkWidget *StepNMI=0;
GtkWidget *BreakMI=0;
GtkWidget *SetBrkMI=0;
GtkWidget *WatchWrMI=0;
GtkWidget *WatchRdMI=0;
GtkWidget *FindMI=0;
GtkWidget *RefreshMI=0;
GtkWidget *NextMI=0;
GtkWidget *PhyDumpMI=0;
GtkWidget *LinDumpMI=0;
GtkWidget *StackMI=0;
GtkWidget *GDTMI=0;
GtkWidget *IDTMI=0;
GtkWidget *PageMI=0;
GtkWidget *ViewBrkMI=0;
GtkWidget *MemDumpMI=0;
GtkWidget *PTreeMI=0;
GtkWidget *DisAsmMI=0;

GtkWidget *ChkMIs[NUM_CHKS]={0};    // menu items with checkmarks

//GtkWidget *XceptMI;
GtkWidget *DefDisMI=0;
GtkWidget *SyntaxMI=0;
GtkWidget *FontMI=0;
GtkWidget *WrdSizeMI=0;
GtkWidget *WSmenu=0;

GtkWidget *AboutMI=0;

GtkWidget *sep1=0;        // menu separators
GtkWidget *sep2=0;
GtkWidget *sep3=0;
GtkWidget *sep4=0;
GtkWidget *sep5=0;
...
GtkWidget *sep8=0;        // separators around the ListViews
GtkWidget *sep9=0;
GtkWidget *sep10=0;

    // GTK widgets
GtkWidget *CmdBtn[6]={0};                   // "command" buttonrow
GtkWidget *CpuBtn[BX_MAX_SMP_THREADS_SUPPORTED]={0};        // "CPU" buttonrow
GtkWidget *CpuB_label[BX_MAX_SMP_THREADS_SUPPORTED]={0};    // "labels" on CPU buttons

GtkWidget *Stat[4]={0};     // 4 entry "Status Bar"
GtkWidget *StatVSep1=0;
GtkWidget *StatVSep2=0;
GtkWidget *StatVSep3=0;

GtkWidget *IEntry=0;      // Singleline Input Text Window
GtkWidget *OText=0;       // Multiline, wrapping, Output Text Window

GtkWidget *PTree=0;       // bochs param_tree TreeView
GtkWidget *LV[4]={0};       // Register, ASM, MemDump / ListViews (TreeViews)

// GTK-specific settings
gint win_x = -1, win_y = -1, win_w=0, win_h=0;
bool window_init = 0;
char fontname[80]={0};
bool font_init = 0;
int FindAsmIndex(int line)
{
    for(int i = 0;i<MAX_ASM;i++)
    {
        if(AsmLine[i]==line) return i;
    }                
    return 0;
}
...
unsigned int CurXSize = 0;      // last known size of main client window
...
unsigned int CurScrX = 0;

//bool UpdInProgress[3];     // flag -- list update incomplete (not OK to paint)

char SelMem[260]={0};       // flag array for which list rows are "selected"
char SelAsm[MAX_ASM]={0};
char SelReg[TOT_REG_NUM + EXTRA_REGS]={0};
...
//L exception notify is not implemented yet
    ...
    NextMI = gtk_menu_item_new_with_label("Next\t\t\t\t\tF12");
...
//  ChkMIs[T_REG] = gtk_check_menu_item_new_with_label("Show Test Registers");
    ...
    sep11 = gtk_separator_menu_item_new();
    ...
    gtk_menu_shell_append(GTK_MENU_SHELL(CmdMenu), NextMI);
...
//  gtk_menu_shell_append(GTK_MENU_SHELL(OptMenu), ChkMIs[T_REG]);
    gtk_menu_shell_append(GTK_MENU_SHELL(OptMenu), sep11);
...
{
    gtk_widget_queue_draw(LV[i]);
            // redraw the selected ListView "bin" window
}
...
void DoSrcListSelect(int index)
{
    ...
    selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(LV[SRCT_WND]));
    GtkTreePath *path;
...
{
    gtk_box_pack_start(GTK_BOX(LVHbox), ScrlWin[REG_WND], FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(LVHbox), VSepEvtBox1, FALSE, FALSE, 0);
    gtk_box_pack_end(GTK_BOX(LVHbox), ScrlWin[DUMP_WND], FALSE, FALSE, 0);
    gtk_box_pack_end(GTK_BOX(LVHbox), VSepEvtBox2, FALSE, FALSE, 0);
    gtk_box_pack_end(GTK_BOX(LVHbox), ScrlWin[SRCT_WND], TRUE, TRUE, 0);
    gtk_box_pack_end(GTK_BOX(LVHbox), VSepEvtBox3, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(LVHbox), ScrlWin[ASM_WND], TRUE, TRUE, 0);
}
...
void VSizeChange()
{
    ...
    else
    {
        ...
        const gchar *gcp;
        gcp = gtk_entry_get_text(GTK_ENTRY(IEntry));
        strncpy (text,gcp,sizeof(text) -1);
        ...
        gtk_entry_set_text(GTK_ENTRY(IEntry),text);
    }
...
//  Invalidate (listnum);
    gtk_widget_show_now(LV[listnum]);
}
...
void FillPTree()
{
    ...
    for (int i=0; i<n; i++)
        MakeBL(NULL, root_param->get(i),tmpcb);
    gtk_widget_show(PTree);
}

void MakeTreeChild (TreeParent *h_P, int ChildCount, TreeParent *h_TC,char* name)
{
    GtkTreeStore *treestore;
    treestore = (GtkTreeStore *) gtk_tree_view_get_model( GTK_TREE_VIEW(PTree) );
    gtk_tree_store_append(GTK_TREE_STORE(treestore), h_TC, h_P);            // create the "leaf"
    gtk_tree_store_set(GTK_TREE_STORE(treestore), h_TC, 0, name, -1);  // and put a name (+ data) on it
}
...
void update_font()
{
  ...
  //g_object_set(G_OBJECT(LV_Rend[3]), "font", fontname, NULL);
...
gboolean SrcMouseDown_cb(GtkWidget *widget, GdkEventButton *event, gpointer data)
{
    GtkTreePath *path;
    gint *pathdat;
    bool 
    onrow = (bool) gtk_tree_view_get_path_at_pos (GTK_TREE_VIEW(LV[3]),
        (gint) (event->x + 0.5), (gint) (event->y + 0.5), &path, NULL, NULL, NULL);
    if (onrow == FALSE)
        return FALSE;
    pathdat = gtk_tree_path_get_indices (path);
    SelectedSrcLine = (int) *pathdat;
    gtk_tree_path_free (path);
...
void CPUb_cb(GtkWidget *widget, gpointer CPUnum)    // "CPUnum" is the value supplied at handler creation
{
    ...
    if (CurrentCPU != newCPU)
    {
        ...
        snprintf(text,sizeof text,"<b>cpu%u</b>",CurrentCPU);
        gtk_label_set_markup (GTK_LABEL(CpuB_label[CurrentCPU]), text);
        // change text on newCPU button to uppercase and BOLD with markup
        snprintf(text,sizeof text,"<b>CPU%u</b>",newCPU);
        gtk_label_set_markup (GTK_LABEL(CpuB_label[newCPU]), text);
        CurrentCPU = newCPU;
...
void Close_cb(GtkWidget *widget, gpointer data)
{
    ...
    gtk_list_store_clear(GTK_LIST_STORE(Database));
    //Database = (GtkListStore *) gtk_tree_view_get_model( GTK_TREE_VIEW(LV[3]) );
    //gtk_list_store_clear(GTK_LIST_STORE(Database));
    treestore = (GtkTreeStore *) gtk_tree_view_get_model( GTK_TREE_VIEW(PTree) );
    gtk_tree_store_clear(GTK_TREE_STORE(treestore));
...
void Src_DblClick (GtkTreeView *treeview, GtkTreePath *path, GtkTreeViewColumn *col, gpointer data)
{
    sizing_cancel();
    if (AtBreak == FALSE)
        return;
...
// activate the menu items
    ...
    g_signal_connect (G_OBJECT(NextMI), "activate", G_CALLBACK(Cmd_cb), (gpointer) CMD_NEXT);
...
//  Cmd2MI[CMD_TREG - CMD_IDX_LO + 1] = ChkMIs[T_REG];
    ...
    g_signal_connect (LV[3], "button_press_event", G_CALLBACK (SrcMouseDown_cb), NULL);
    g_signal_connect (LV[3], "row-activated", G_CALLBACK(Src_DblClick), NULL);
    ...
    g_signal_connect (G_OBJECT (LV[3]), "button_release_event", G_CALLBACK (LV_MouseUp), NULL);
}
...
void MakeList(int listnum, const char *ColNameArray[],int sign = 1)
{
    int i;
    int nColumns = 3;
    char txt[4];
    ...
    *txt = '0' -1;
    ...
    for (i=0 ; i < nColumns ; i++)
    {
        ...
        gtk_tree_view_column_set_cell_data_func(column, renderer, ListClr_PaintCb, (gpointer) (glong) (((listnum*3) + i)*sign), NULL);
...
//  gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(ScrlWinOut), OText);
    ...
    VSizeChange();
    // build the 4 ListView windows
    i = 3;   
    const char* LColNamesLines[] = {
        "Line","Text",""
    };
    ...
    //gtk_widget_set_size_request(ScrlWin[i],320,240);

    MakeList(i, LColNamesLines, -1);
...
// all TreeViews *already have* a viewport, so DON'T add another one with the ScrollWindow, or it won't scroll properly!
    ...
    gtk_event_box_set_above_child (GTK_EVENT_BOX(VSepEvtBox2), TRUE);
    gtk_event_box_set_visible_window (GTK_EVENT_BOX(VSepEvtBox3), FALSE);
    gtk_event_box_set_above_child (GTK_EVENT_BOX(VSepEvtBox3), TRUE);
...
void MakeBL(TreeParent *h_P, bx_param_c *p, char* name)
{
    TreeParent h_new;
    ...
    switch (p->get_type())
    {
        case BXT_LIST:
            ...
            n = as_list->get_size();
            break;
            sprintf(name + j,": %s",((bx_param_bool_c*)p)->get()?"true":"false");
            break;
            sprintf(name + j,": %s",((bx_param_enum_c*)p)->get_selected());
            break;
        case BXT_PARAM_BOOL:
        case BXT_PARAM_ENUM:
        case BXT_PARAM_NUM:
        ...
        case BXT_PARAM_BYTESTRING:
            ...
            sprintf(name + j,": %s", tmpstr);
            break;
        case BXT_PARAM_DATA:
            sprintf (name + j,": binary data, size=%d",((bx_shadow_data_c*)p)->get_size());
            break;
    }
    MakeTreeChild (h_P, n, &h_new,name);
    if (n > 0)
    {
        for (int i=0; i<n; i++)
            MakeBL(&h_new, as_list->get(i),name);    // recurse for all children that are lists
    }
}
