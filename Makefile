...
CC = gcc
CXX = g++
CFLAGS = -g -D_FILE_OFFSET_BITS=64 -D_LARGE_FILES -pthread  $(X_CFLAGS) $(MCH_CFLAGS) $(FLA_FLAGS)  -DBX_SHARE_PATH='"$(sharedir)"'
CXXFLAGS = -g -O2 -D_FILE_OFFSET_BITS=64 -D_LARGE_FILES -pthread  $(X_CFLAGS) $(MCH_CFLAGS) $(FLA_FLAGS)  -DBX_SHARE_PATH='"$(sharedir)"'

LDFLAGS = 
LIBS =  -lgtk-x11-2.0 -lgdk-x11-2.0 -lpangocairo-1.0 -latk-1.0 -lcairo -lgdk_pixbuf-2.0 -lgio-2.0 -lpangoft2-1.0 -lpango-1.0 -lgobject-2.0 -lglib-2.0 -lharfbuzz -lfontconfig -lfreetype -lbfd -ldl -lz -liberty
...
GUI_LINK_OPTS_NOGUI =
GUI_LINK_OPTS_TERM = -Wl,-Bsymbolic-functions -lncurses -ltinfo
GUI_LINK_OPTS_WX = 
...
bochs: iodev/libiodev.a iodev/display/libdisplay.a iodev/hdimage/libhdimage.a \
		$(DEBUGGER_LIB) cpu/libcpu.a $(AVX_LIB) cpu/cpudb/libcpudb.a memory/libmemory.a \
	...
	$(LIBTOOL) --mode=link --tag CXX $(CXX) -o $@ $(CXXFLAGS) $(LDFLAGS) -export-dynamic $(BX_OBJS) $(SIMX86_OBJS) \
		...
		$(DEBUGGER_LIB) cpu/libcpu.a $(AVX_LIB) cpu/cpudb/libcpudb.a \
